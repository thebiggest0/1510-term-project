...
class TestForce(TestCase):
    ...
    @patch('character_actions.combat.random_calculate_generator', return_value=('What is 7 + 3? ', 10))
    def test_attack_force_incorrect_answer(self, _, __):
        character_data = {'name': 'Jedi Knight', 'int': 5, 'health': 100, 'attack': 25, 'defense': 15}
        result = attack_force(character_data)
        self.assertEqual(result, 1)

    @patch('builtins.input', side_effect=['10'])
    @patch('character_actions.combat.random_calculate_generator', return_value=('What is 5 * 2? ', 10))
